# -*- coding: utf-8 -*-
"""
Окно управления пользователями - ТОЛЬКО ДЛЯ ADMIN
"""
import json
import bcrypt
...
class UsersWindow(QWidget):
    """Окно управления пользователями - ДОСТУПНО ТОЛЬКО ADMIN"""

    USERS_FILE = "users.json"

    def __init__(self, current_user_role=None):  # Добавляем параметр с значением по умолчанию
        super().__init__()
        self.current_user_role = current_user_role or "user"  # Если None, то "user"
    ...
    def init_ui(self):
        ...
        # Кнопка добавления (доступна только admin)
    ...
    def update_table(self):
        ...
        for row, (username, data) in enumerate(self.users.items()):
            ...
            if data.get('role') == 'admin':
                role_item.setForeground(QColor(styles.S7_GREEN))
            ...
            if self.current_user_role == 'admin':
                ...
                edit_btn.setToolTip("Редактировать роль")
                btn_layout.addWidget(edit_btn)

                # Кнопка удаления (не для admin)
                if username != 'admin':
                ...
                # Для user показываем только просмотр
                self.table.setItem(row, 2, QTableWidgetItem("Только просмотр"))

    def add_user(self):
        if self.current_user_role != 'admin':
            QMessageBox.warning(self, "Доступ запрещен", "Только администратор может добавлять пользователей")
            return
    ...
    def edit_user(self, username):
        if self.current_user_role != 'admin':
            QMessageBox.warning(self, "Доступ запрещен", "Только администратор может редактировать пользователей")
            return
    ...
    def delete_user(self, username):
        if self.current_user_role != 'admin':
            QMessageBox.warning(self, "Доступ запрещен", "Только администратор может удалять пользователей")
            return
